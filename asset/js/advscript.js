...
var starturl =  url;
function check_main_sub_view(){
  url = window.location.pathname;
  var querysplit = url.split('/');
  if(url){
    mainview = querysplit[querysplit.length-1];
    if(querysplit.length > 1){
      subview =  querysplit[querysplit.length-2];
    }
  }
}
check_main_sub_view();
...
function onPageshow(anycondition,url2load, fnc2set,nowaitsocket=false) {
  ...
  var last_path = querysplit[querysplit.length -1];

  var registerurl = '/' + last_path;
...
function triggerPageshow(anycondition) { // Register function
  ...
  var lastpath = anycondition.split('/');
  ...
  for (var x = 0; x < allpage_register.length; x++) {
      ...
      //console.log("condition" + lastpath ,allpage_register[x].condition)
      if(allpage_register[x].condition == window.location.pathname || allpage_register[x].condition ==lastpath){
...
function go2View(anyinternalurl, titlename) {
  ...
  if (anyinternalurl.indexOf('/') !== 0) { // First String is '/'  not found at first position
    anyinternalurl = '/' + anyinternalurl;
  }
  var thepage  = anyinternalurl.split('/');
  var lastpath  = thepage[thepage.length - 1];
  console.log("last Path to go" + lastpath)

  var registerurl =  anyinternalurl;

  if (!titlename) titlename = anyinternalurl;
...
if(!firsttimevisitwebsite){
  ...
  check_main_sub_view();      // Main View & Sub View Change
}
