    ...
    make_edittextarea: function (jqueryobject, callback, validate) {
        jqueryobject.editable({
            ...
            success: function (response, newValue) {
                ...
                if (!id)
                    id = $(this).parents('.row_tablecontent').attr('data-id');
                var field = $(this).attr('data-field');
                var update = {_id: id};
                if (!field)
                    return console.log('Object has no attribute data-field');
    ...
    make_edittext: function (jqueryobject, callback, validate) {
        ...
        jqueryobject.editable({
            ...
            success: function (response, newValue) {
                ...
                var update = {_id: id};
                if (!field)
                    return console.log('Object has no attribute data-field2');
                update[field] = newValue;
                return callback(update, $(this));
            }
        });
        ...
        if (!jq_obj.attr('placeholder'))
            jq_obj.attr('placeholder', 'Select..');
        if (typeof (setting) != 'object')
            return alert("Must Have setting");
        if (!setting.url)
            return alert("Must Have setting.url");
        if (!setting.postdata)
            return alert("Must Have setting.postdata");
